...
bash "import access_trottoir" do
  ...
  cwd "#{node['parisaccessible']['home']}"
  code <<-EOH
  java -jar -Xms2048m -Xmx2048m -Dparisaccessible_home=#{node['parisaccessible']['home']} ParisAccessibleInjector/target/*.war --access_trottoir trottoir\\.csv\\.+ > #{node['parisaccessible']['log']}/inject.access_trottoir.log  
  EOH
end
...
bash "import access_passagepieton" do
  ...
  cwd "#{node['parisaccessible']['home']}"
  code <<-EOH
  java -jar -Xms2048m -Xmx2048m -Dparisaccessible_home=#{node['parisaccessible']['home']} ParisAccessibleInjector/target/*.war --access_passagepieton passagepieton\\.csv\\.+ > #{node['parisaccessible']['log']}/inject.access_passagepieton.log  
  EOH
end
